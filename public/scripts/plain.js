  ...
  $(document).ready(function(){
	  ...
	  $('#editor').bind({
	    ...
	    edit: function(evt, entry) {
	      ...
	      jQuery.ajax(data_path + entry.attr('id'), {dataType: 'json', success: function(data){
	    ...
	    save: function(evt) {
	      ...
	      var editor = $(this),
		    ...
		    if(!entry.hasClass('creating'))		
		      jQuery.ajax(data_path + entry.attr('id'), {
			      type: 'PUT', 
          ...
          jQuery.ajax(data_path + parent_id, {		  
	          type: 'POST', 
	          data: JSON.stringify({
	          ...
	          success: function(data){
		          entry.removeClass('creating')
		               .data('entry', data).trigger('content-updated');
		          editor.trigger('close');		        
	          }
          });	      
	      }
    ...
    $('#editor .close').click(function() { $('#editor').trigger('close'); return false; });
	  ...
	  $.ajax(data_path, {success: function(data){
	  ...
	  $('.entry > aside > .actions > .delete', map_node).live('click', function(){
		  ...
		  jQuery.ajax(data_path + id, {type: 'DELETE', success: function(data, status, xhr){
			  entry.remove();
		  }});
	  });
